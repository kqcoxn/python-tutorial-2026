...
本文通过审核后，才开始批量制作教师教案、学生讲义、示例、练习和 PPT。
...
```mermaid
flowchart LR
    M0["M0 课程介绍与编程概述"] --> M1["M1 运行环境与 Debug"]
    M1 --> M2["M2 值、变量与表达式"]
    M2 --> M3["M3 条件判断"]
    M3 --> M4["M4 循环"]
    M4 --> M5["M5 函数"]
...
- M0 固定为课程开篇。
- M1 应在首次独立编程前完成，但 Debug 方法在后续每个模块重复使用。
- M2 是 M3–M7 的共同基础。
...
| M1 · 运行环境与 Debug | 25 | 35 | 50 | 能独立运行程序并完成一次有依据的排错 |
| M2 · 值、变量与表达式 | 50 | 70 | 90 | 能实现包含输入、计算和格式化输出的小程序 |
...
- 能创建、保存、运行和重新运行一个程序。
- 认识 Traceback、错误类型、行号和最后一行错误信息。
- 认识 `SyntaxError`、`NameError`、`TypeError`、`ValueError` 等典型错误；具体错误在相关语法出现后复现。

**思维目标**

- 把报错和异常输出当作证据，而不是失败结论。
- 形成“观察—提出一个假设—只改一处—重新验证”的排错流程。
- 区分程序无法运行和程序运行但结果错误。
...
| 模块 | 最低档必须保留 | 从最低档移出 |
| --- | --- | --- |
| M0 | 问题、算法、程序；输入—处理—输出；一次无代码拆解 | 大学应用领域展开、多个类比活动 |
| M1 | 独立运行、读错误最后一行、一次最小排错 | 断点单步、多个错误类型集中演示 |
| M2 | 四种基本类型、变量、输入输出、必要转换、核心运算、f-string | 复杂转义、浮点现象扩展、更多格式细节 |
...
M0 至少完成一次无代码问题拆解；M1 至少完成一次独立运行和一次有依据的排错。启用最低档时，活动合并规则见第 6 节。
...
| 证据 | 对应能力 |
| --- | --- |
| M1 运行与排错检查点 | 环境确认、运行、错误信息和验证流程 |
| M2 输入—计算—输出任务 | 变量、类型、转换、表达式求值和格式化输出 |
...
| 天次 | 建议内容 | 分钟 | 当天检查点 | 助教 |
| ---: | --- | ---: | --- | --- |
| 1 | M0 30 + M1 35 + M2 20 + 机动 15 | 100 | 能独立运行程序，并追踪一次输入、赋值和输出 | 梁健、谢雨荷 |
| 2 | M2 50 + M3 45 + 机动 5 | 100 | 能完成基本分支，并开始检查边界 | 梁健 |
...
1. M0–M8 的模块划分是否合适。
2. 550 分钟内容加 50 分钟机动的比例是否合适。
3. 函数先于系统容器教学的推荐顺序是否保留。
4. 全课程主动排除项是否需要调整。
5. 最低成果证据是否足以验证课程目标。
6. 六天推荐路线是否适合作为正常进度基准。
