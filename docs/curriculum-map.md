...
本文已经完成课程结构审核。M0 教师教案已经通过审核，当前先制作 M0 PPT 内容稿；内容稿和练习完成审核后，再批量制作其余模块的教师教案、PPT 内容稿和练习。
...
课程采用“群星彼岸：宜居行星探索计划”作为暂定贯穿式案例。案例以飞船上的“行星气候沙盘”为科研内核，使用科研探索型文字游戏组织主要实践：学生扫描候选星球、分析宜居条件与风险、选择航线和行动，并最终提交宜居行星候选及证据说明。

主题负责提供统一语境和持续演进的任务，不改变本蓝图的模块依赖、知识边界和时间预算，也不要求一个程序承载全部成果证据。详细主题决定、科学与科幻边界及后续待定项见 [`project-theme.md`](project-theme.md)，逐课时项目拆解、成果评价和自由扩展初稿见 [`project-implementation-plan.md`](project-implementation-plan.md)。
...
不单独制作学生讲义、PPT 实现或示例代码目录。学生可见的内容稿不暴露内部学生画像、教师答案和进度删减规则；其中的代码示例必须能够运行，故障示例必须明确标记为故意错误。
...
课程蓝图已满足进入 M0 样板制作的条件。贯穿式案例主题已经确定；具体任务、科学指标、游戏状态和模块映射仍需在后续设计阶段审核。PPT 最终视觉实现继续在相应样板阶段单独确认。
