# -*- coding: utf-8 -*-
"""Assemble lessons/09-for-break-continue/slides.html.
   管道与 lesson04-build/build.py 相同：viewport-base + 内联字体 + 三风格 CSS
   + 课件通用补充样式（连字禁用 + C 揭示条，与第 8 章同款）+ chrome CSS/DOM
   + 28 页幻灯片 + 补丁后的可编辑运行时 + 课件专属交互（data-reveal / data-step）。
   共享片段（fonts-inline.css / frag-style.css）仍从 .design/fonts-tmp 读取。"""
import io, os

BASE = os.path.dirname(os.path.abspath(__file__))
ROOT = os.path.normpath(os.path.join(BASE, '..', '..'))
REF = os.path.join(ROOT, '.agents', 'skills', 'frontend-slides-editable', 'examples', 'editable-deck-reference.html')
OUT = os.path.join(ROOT, 'lessons', '09-for-break-continue', 'slides.html')
...
lesson_css = read(os.path.join(BASE, 'frag-style-09.css'))
slides_html = read(os.path.join(BASE, 'frag-slides.html')).strip()
extra_js = read(os.path.join(BASE, 'frag-extra.js')).strip()

html = f"""<!DOCTYPE html>
<html lang="zh-CN" data-deck-id="py09-for-break-continue-v1" data-template-edit-mode="components" data-mobile-adaptation="desktop-default">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>第 9 章 · for 循环与 break、continue — 课件</title>
<style>
...
{custom_css}
/* === SECTION: 课件通用补充样式（连字禁用 + C 揭示条，全套章节沿用） === */
{lesson_css}
