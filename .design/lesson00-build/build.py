# -*- coding: utf-8 -*-
"""Assemble slides.html: viewport-base + inline fonts + custom CSS (+ lesson00 CSS)
   + chrome CSS/DOM + 13 slides + full editable runtime (patched) + deck-specific interactions.
   Lesson 00 · 课程导入。共享片段（fonts-inline.css / frag-style.css）仍从 .design/fonts-tmp 读取。"""
import io, os, re, sys

BASE = os.path.dirname(os.path.abspath(__file__))
ROOT = os.path.normpath(os.path.join(BASE, '..', '..'))
REF = os.path.join(ROOT, '.agents', 'skills', 'frontend-slides-editable', 'examples', 'editable-deck-reference.html')
OUT = os.path.join(ROOT, 'lessons', '00-course-intro', 'slides.html')
...
lesson_css = read(os.path.join(BASE, 'frag-style-00.css'))
slides_html = read(os.path.join(BASE, 'frag-slides.html')).strip()
extra_js = read(os.path.join(BASE, 'frag-extra.js')).strip()

html = f"""<!DOCTYPE html>
<html lang="zh-CN" data-deck-id="py00-course-intro-v1" data-template-edit-mode="components" data-mobile-adaptation="desktop-default">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>第 0 章 · 课程导入 — 课件</title>
<style>
