# -*- coding: utf-8 -*-
"""Assemble slides.html: viewport-base + inline fonts + custom CSS + chrome CSS/DOM
   + 16 slides + full editable runtime (patched) + deck-specific interactions."""
import io, os, re, sys

BASE = os.path.dirname(os.path.abspath(__file__))
ROOT = os.path.normpath(os.path.join(BASE, '..', '..'))
REF = os.path.join(ROOT, '.agents', 'skills', 'frontend-slides-editable', 'examples', 'editable-deck-reference.html')
OUT = os.path.join(ROOT, 'lessons', '01-environment-first-program', 'slides.html')
...
# --- 4. patch: strip click-reveal state from saved/exported decks ---
...
fonts_css = read(os.path.join(BASE, 'fonts-inline.css'))
custom_css = read(os.path.join(BASE, 'frag-style.css'))
slides_html = read(os.path.join(BASE, 'frag-slides.html')).strip()
extra_js = read(os.path.join(BASE, 'frag-extra.js')).strip()

html = f"""<!DOCTYPE html>
<html lang="zh-CN" data-deck-id="py01-env-first-program" data-template-edit-mode="components" data-mobile-adaptation="desktop-default">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>第 1 章 · 环境、Python 与第一个程序 — 课件</title>
<style>
